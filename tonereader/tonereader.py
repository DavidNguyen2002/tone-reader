...
def is_sarcastic(text: str, n: int):
    ...
    df_ngram_prev_count_notsarcastic = pd.read_csv(f"tonereader/data/ngram_prev_notsarcastic_count_{n}.csv", converters={"unique_values": literal_eval})
    
    return is_sarcastic_helper(text, n, df_ngram_count_sarcastic, df_ngram_count_notsarcastic, df_ngram_prev_count_sarcastic, df_ngram_prev_count_notsarcastic)

def is_sarcastic_helper(text: str, n: int, df_ngram_count_sarcastic, df_ngram_count_notsarcastic, df_ngram_prev_count_sarcastic, df_ngram_prev_count_notsarcastic):
    nlp = spacy.load("en_core_web_sm")
    ...
    for ngram in text_ngrams:
        ...
        if (idx_ngram_sar == -1 or idx_ngram_prev_sar == -1) and (idx_ngram_notsar == -1 or idx_ngram_prev_notsar == -1):
            continue
...
def test_ngrams(test_csv: str, n: int):
    ...
    def get_result(row):
        return is_sarcastic_helper(row['comment'], n, df_ngram_count_sarcastic, df_ngram_count_notsarcastic, df_ngram_prev_count_sarcastic, df_ngram_prev_count_notsarcastic) == row['label']
